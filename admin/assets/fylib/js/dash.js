$(function() {
	...
	metrics_charts: function() {
		...
		if ($(r).length) {
			function e() {
				if (0 == i) var i = $(r).height();
				var n = $(r).width();
				d3.json("data/mg_dashboard_chart.min.json").then(function(e) {
					e = [e];
					for (var t = 0; t < e.length; t++) e[t] = MG.convert.date(e[t], "date");
					var a = [{
		...
		if (e.length) {
			...
			$("#peity_live_text").text("0"), setInterval(function() {
				var e = Math.round(10 * Math.random()),
					t = n.text().split(",");
				t.shift(), t.push(e), n.text(t.join(",")).change();
				var a = parseInt($("#peity_live_text").text());
				if (a == (i = r(20, 100))) var i = r(20, 120);
				new CountUp("peity_live_text", a, i, 0, 1.2).start()
			}, 2e3)
		}
	},
	count_animated: function() {
		$(".countUpMe").each(function() {
			var e = $(this).text();
			theAnimation = new CountUp(this, 0, e, 0, 2), theAnimation.start()
		})
	},
	clndr_calendar: function() {
		var i = $("#clndr_events");
		if (i.length) {
			for (var e = $("#clndr_events_template").html(), t = Handlebars.compile(e), a = [], n = 0; n < 7; n++) a.push(moment().weekday(n).format("dd"));
			theCalendar = i.clndr({
				weekOffset: 1,
				daysOfTheWeek: a,
				events: clndrEvents,
					...
					click: function(e) {
						if (e.events.length) {
							...
							$(e.element).siblings(".day").removeClass("day-active").end().addClass("day-active"), t.children("[data-clndr-event=" + a + "]").length ? (t.children(".clndr_event").hide(), i.hasClass("events_visible") ? t.children("[data-clndr-event=" + a + "]").velocity("transition.slideUpIn", {
								stagger: 100,
								drag: !0
							}) : (l(), i.addClass("events_visible"), t.children("[data-clndr-event=" + a + "]").velocity("transition.slideUpIn", {
								stagger: 100,
								drag: !0,
								delay: 280
			...
			}), event_modal = UIkit.modal("#modal_clndr_new_event"), i.on("click", ".clndr_add_event", function() {
				event_modal.isActive() ? event_modal.hide() : (event_modal.show(), i.removeClass("events_visible"), setTimeout(function() {
					$window.resize()
				}, 280))
			...
			l(), $(window).on("debouncedresize", function() {
				l()
			})
		}
	},
	video_player: function() {
		var i = $("#video_player"),
			n = $("#video_player_playlist"),
			r = "md-list-item-active";
		i.length && (n.on("click", "li", function(e) {
			e.stopPropagation();
			var t = $(this);
			if (!t.hasClass(r)) {
				var a = "https://www.youtube.com/embed/" + t.attr("data-video-src") + "?rel=0";
				n.children("li").removeClass(r), t.addClass(r), i.velocity({
					translateZ: 0,
					scale: 0,
					opacity: 0
				}, {
					...
					complete: function() {
						i.children("iframe").attr("src", a), setTimeout(function() {
							i.velocity("reverse")
						}, 280)
					}
				})
			}
		}), n.children("li:first").trigger("click"))
	}
};