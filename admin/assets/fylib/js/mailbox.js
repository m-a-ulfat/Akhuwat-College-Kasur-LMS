...
altair_mailbox = {
	init: function() {
		altair_mailbox.list_switch(), altair_mailbox.list_category_color(), altair_mailbox.select_messages(), altair_mailbox.toggle_message(), altair_mailbox.create_message()
	},
	list_switch: function() {
		$combined_view_btn.on("click", function(i) {
			i.preventDefault(), i.stopPropagation(), $combined_view_btn.velocity("transition.expandOut", {
		...
		}), $split_view_btn.on("click", function(i) {
			i.preventDefault(), $split_view_btn.velocity("transition.expandOut", {
				duration: anim_duration,
				easing: easing_swiftOut,
				begin: function() {
	...
	list_category_color: function() {
		$(".md-card-list > ul > li").each(function() {
			$this = $(this);
			var i = $this.attr("data-message-category-color");
			i && $(this).find("span.md-card-list-item-avatar").css({
				backgroundColor: "#" + i
			})
		})
	},
	select_messages: function() {
		$mailbox.on("ifChanged", "[data-md-icheck]", function() {
			...
			$mailbox.find("[data-md-icheck]").each(function() {
				i.is(":checked") ? $(this).iCheck("check") : $(this).iCheck("uncheck")
			})
		})
	},
	...
	create_message: function() {
		...
		var t = $("#mail_progressbar"),
			e = t.find(".uk-progress-bar"),
			i = {
				action: "./upload/",
				single: !1,
				loadstart: function() {
					e.css("width", "0%").text("0%"), t.removeClass("uk-hidden uk-progress-danger")
				},
				progress: function(i) {
					i = Math.ceil(i), e.css("width", i + "%").text(i + "%"), "100" == i && setTimeout(function() {}, 1500)
				},
				error: function(i) {
					t.addClass("uk-progress-danger"), e.css({
						width: "100%"
					}).text("100%")
				},
				abort: function(i) {
		...
		UIkit.uploadSelect($("#mail_upload-select"), i), UIkit.uploadDrop($("#mail_upload-drop"), i)
	}
};