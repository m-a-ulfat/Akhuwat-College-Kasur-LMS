...
function scrollbarWidth() {
	var e = jQuery('<div style="width: 100%; height:200px;">test</div>'),
		...
		a = e[0];
	e = i[0];
	return jQuery("body").append(e), a = a.offsetWidth, i.css("overflow", "scroll"), e = e.clientWidth, i.remove(), a - e
}

function randID_generator() {
	return String.fromCharCode(65 + Math.floor(26 * Math.random())) + Date.now()
}

function detectIE() {
	var e = window.navigator.userAgent,
		i = e.indexOf("MSIE ");
	return 0 < i ? parseInt(e.substring(i + 5, e.indexOf(".", i)), 10) : 0 < e.indexOf("Trident/") ? (i = e.indexOf("rv:"), parseInt(e.substring(i + 3, e.indexOf(".", i)), 10)) : 0 < (i = e.indexOf("Edge/")) && parseInt(e.substring(i + 5, e.indexOf(".", i)), 10)
}

function hex2rgba(e, i) {
	return e = e.replace("#", ""), r = parseInt(e.substring(0, 2), 16), g = parseInt(e.substring(2, 4), 16), b = parseInt(e.substring(4, 6), 16), result = "rgba(" + r + "," + g + "," + b + "," + i / 100 + ")", result
}
...
$(function() {
...
}, void 0 !== $.fn.selectize && (Selectize.define("dropdown_after", function(e) {
	...
	var a, i = this;
	this.setup = (a = i.setup, function() {
		a.apply(this, arguments);
		var e = this.$wrapper,
			i = this.$input;
		i.attr("title") && e.attr("title", i.attr("title")).attr("data-uk-tooltip", i.attr("data-uk-tooltip"))
	})
	...
	if ("multi" === this.settings.mode) {
		...
		}), n.unlock = (a = n.unlock, function() {
			var e = n.$control.data("sortable");
			return e && e.enable(), a.apply(n, arguments)
		}), n.setup = (t = n.setup, function() {
			t.apply(this, arguments);
			var a = n.$control.sortable({
				items: "[data-value]",
				forcePlaceholderSize: !0,
				disabled: n.isLocked,
				start: function(e, i) {
					i.placeholder.css("width", i.helper.css("width")), a.css({
						overflow: "visible"
					})
				},
				stop: function() {
					...
					var e = n.$activeItems ? n.$activeItems.slice() : null,
						i = [];
					a.children("[data-value]").each(function() {
						i.push($(this).attr("data-value"))
					}), n.setValue(i), n.setActiveItem(e)
				},
				helper: function(e, i) {
					var a = $(i).clone();
					return a.css("position", "absolute"), a.get(0)
				}
			})
		})
	}
	...
	var s, i, a, t, r, n = function(e) {
		return (e + "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;")
	};
	"single" !== this.settings.mode ? (t = s = this, r = '<a href="javascript:void(0)" class="' + (i = e).className + '" tabindex="-1" title="' + n(i.title) + '">' + i.label + "</a>", s.setup = (a = t.setup, function() {
		if (i.append) {
			var n = t.settings.render.item;
			t.settings.render.item = function(e) {
				return i = n.apply(s, arguments), a = r, t = i.search(/(<\/[^>]+>\s*)$/), i.substring(0, t) + a + i.substring(t);
				var i, a, t
			}
		}
		a.apply(s, arguments), s.$control.on("click", "." + i.className, function(e) {
			if (e.preventDefault(), !t.isLocked) {
				var i = $(e.currentTarget).parent();
				t.setActiveItem(i), t.deleteSelection() && t.setCaret(t.items.length)
			}
		})
	})) : function(s, i) {
		i.className = i.className || "remove-single";
		var a, t = s,
			r = '<a href="javascript:void(0)" class="' + i.className + '" tabindex="-1" title="' + n(i.title) + '">' + i.label + "</a>";
		s.setup = (a = t.setup, function() {
			if (i.append) {
				var e = $(t.$input.context).attr("id"),
					n = ($("#" + e), t.settings.render.item);
				t.settings.render.item = function(e) {
					return i = n.apply(s, arguments), a = r, t = i.search(/(<\/[^>]+>\s*)$/), i.substring(0, t) + a + i.substring(t);
					var i, a, t
				}
			...
			a.apply(s, arguments), s.$control.on("click", "." + i.className, function(e) {
				e.preventDefault(), t.isLocked || t.clear()
			})
		})
	}(this, e)
...
altair_page_onload = {
	init: function() {
		$window.on("load", function() {
			altair_helpers.hierarchical_show(), altair_helpers.hierarchical_slide(), $body.removeClass("disable_transitions")
		})
	}
}, altair_page_content = {
	...
	init: function() {
		altair_forms.textarea_autosize(), altair_forms.select_elements(), altair_forms.select_elements_2(), altair_forms.switches(), altair_forms.dynamic_fields()
	},
	textarea_autosize: function() {
		$textarea = $("textarea.md-input").not(".no_autosize"), $textarea.hasClass("autosized") && autosize.destroy($textarea), autosize($textarea), $textarea.addClass("autosized")
	},
	select_elements: function(e) {
		(e ? $(e).find("[data-md-selectize-delayed]") : $("[data-md-selectize],.data-md-selectize")).each(function() {
			var i = $(this);
			if (!i.hasClass("selectized")) {
				var a = i.attr("data-md-selectize-bottom");
				i.after('<div class="selectize_fix"></div>').selectize({
					plugins: ["tooltip"],
					hideSelected: !0,
					dropdownParent: "body",
					...
					onChange: function(e) {
						i.attr("data-parsley-id") && i.parsley().validate()
					},
					onInitialize: function() {
						i[0].selectize.isRequired && i.prop("required", !0)
					}
				})
			}
		}), (e ? $(e).find("[data-md-selectize-inline-delayed]") : $("[data-md-selectize-inline]")).each(function() {
			var i = $(this);
			if (!i.hasClass("selectized")) {
				var a = i.attr("data-md-selectize-bottom");
				i.after('<div class="selectize_fix"></div>').closest("div").addClass("uk-position-relative").end().selectize({
					plugins: ["dropdown_after"],
					dropdownParent: i.closest("div"),
					hideSelected: !0,
					onDropdownOpen: function(e) {
						e.hide().velocity("slideDown", {
							begin: function() {
	...
	select_elements_2: function(e) {
		(e ? $(e).find("[data-md-select2-delayed]") : $("[data-md-select2],.md-select2")).each(function() {
			var e = $(this);
			e.hasClass("select2-hidden-accessible") || e.select2({
				closeOnSelect: !1
			}), e.on("select2:open", function(e) {
				var i = $(".select2-dropdown");
	...
	switches: function(e) {
		var i = e ? $(e).find("[data-switchery]") : $("[data-switchery]");
		i.length && i.each(function() {
			if (!$(this).siblings(".switchery").length) {
				var e = $(this).attr("data-switchery-size"),
					i = $(this).attr("data-switchery-color"),
					a = $(this).attr("data-switchery-secondary-color"),
					t = new Switchery(this, {
						color: hex2rgba(void 0 !== i ? i : "#009688", 50),
						jackColor: hex2rgba(void 0 !== i ? i : "#009688", 100),
						secondaryColor: void 0 !== a ? hex2rgba(a, 50) : "rgba(0, 0, 0,0.26)",
						jackSecondaryColor: void 0 !== a ? hex2rgba(a, 50) : "#fafafa",
						className: "switchery" + (void 0 !== e ? " switchery-" + e : "")
					});
				$(this).data("ObjSwitchery", t)
			}
		})
	},
	parsley_validation_config: function() {
		window.ParsleyConfig = {
			...
			classHandler: function(e) {
				var i = e.$element;
				if (i.is(":checkbox") || i.is(":radio") || i.parent().is("label") || $(i).is("[data-md-selectize]")) return i.closest(".parsley-row")
			}
		}
	},
		...
		window.ParsleyConfig = window.ParsleyConfig || {}, window.ParsleyConfig.validators = window.ParsleyConfig.validators || {}, window.ParsleyConfig.validators.date = {
			fn: function(e) {
				...
				var i = e.split(/[.\/-]+/),
					...
					t = parseInt(i[0], 10),
					n = parseInt(i[2], 10);
				if (0 == n || 0 == t || 12 < t) return !1;
				var s = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
				return (n % 400 == 0 || n % 100 != 0 && n % 4 == 0) && (s[1] = 29), 0 < a && a <= s[t - 1]
			},
			priority: 256
		}
	},
	dynamic_fields: function(e, n, i) {
		function s(e, i, a) {
			var t = $("#" + i).html(),
			...
			e.append(n), altair_md.inputs(e), altair_md.checkbox_radio(e.find("[data-md-icheck]")), altair_forms.switches(e), altair_forms.select_elements(e)
			...
			i || s(a, t), a.on("click", ".btnSectionClone", function(e) {
				e.preventDefault(), a.find(".btnSectionClone").replaceWith('<a href="#" class="btnSectionRemove"><i class="material-icons md-24">&#xE872;</i></a>');
				var i = parseInt(a.attr("dynamic-fields-counter")) + 1;
				a.attr("dynamic-fields-counter", i), s(a, t, i), n && $window.resize()
			}).on("click", ".btnSectionRemove", function(e) {
				e.preventDefault(), $(this).closest(".form_section").next(".form_hr").remove().end().remove(), n && $window.resize()
			})
		})
	}
}, altair_main_sidebar = {
	init: function() {
		$sidebar_main.length && (lsTest() && (null !== localStorage.getItem("altair_sidebar_mini") && $body.addClass("sidebar_mini"), null !== localStorage.getItem("altair_sidebar_slim") && $body.addClass("sidebar_slim")), $body.hasClass("sidebar_mini") ? (altair_main_sidebar.mini_sidebar(), setTimeout(function() {
			...
			e.preventDefault(), $body.hasClass("sidebar_main_active") || $body.hasClass("sidebar_main_open") && 1220 <= $window.width() ? altair_main_sidebar.hide_sidebar() : altair_main_sidebar.show_sidebar()
		...
		}), altair_helpers.custom_scrollbar($sidebar_main), $body.hasClass("sidebar_main_active") && $window.width() < 1220 ? altair_page_content.hide_content_sidebar() : altair_page_content.show_content_sidebar(), $body.hasClass("sidebar_main_open") && $window.width() < 1220 && $body.removeClass("sidebar_main_open"), altair_main_sidebar.sidebar_menu(), altair_main_sidebar.swipe_open()), altair_main_sidebar.lang_switcher())
	},
	...
	show_sidebar: function() {
		$body.addClass("sidebar_main_active"), $window.width() < 1220 && !$body.find("#page_overflow").length && altair_page_content.hide_content_sidebar(), setTimeout(function() {
			$window.resize()
		}, 290)
	},
	sidebar_menu: function() {
		$sidebar_main.find(".menu_section > ul").find("li").each(function() {
			$(this).children("ul").length && $(this).addClass("submenu_trigger")
		...
		}), $sidebar_main.find(".act_item").each(function() {
			var e = $(this);
			e.parents(".submenu_trigger").last().addClass("current_section"), e.parents(".submenu_trigger").addClass("act_section").children("ul").show(), e.children("ul").show()
		})
	},
	...
	mini_sidebar: function() {
		$sidebar_main_toggle.hide(), $sidebar_main.find(".menu_section > ul").children("li").each(function() {
			$(this).children("ul").length ? ($(this).addClass("sidebar_submenu"), $(this).find(".act_item").length && $(this).addClass("current_section")) : $(this).attr({
	...
	init: function() {
		$sidebar_secondary.length && ($sidebar_secondary_toggle.removeClass("sidebar_secondary_check"), $sidebar_secondary_toggle.on("click", function(e) {
			...
			$body.hasClass("sidebar_secondary_persisten") || !$body.hasClass("sidebar_secondary_active") || ($(e.target).closest($sidebar_secondary).length || $(e.target).closest($sidebar_secondary_toggle).length) && 27 != e.which || altair_secondary_sidebar.hide_sidebar()
	...
	init: function() {
		$topBar.length && ($topBar.hasClass("top_bar_static") ? $body.addClass("top_bar_static_active") : $body.addClass("top_bar_active"))
	}
}, altair_page = {
	_heading: function() {
	...
	_aside: function() {
		if ($pageAside.length) {
			function e() {
				var e = $window.height(),
					i = $pageAside.offset().top;
				$pageAside.height(e - i)
			}
			e(), $window.on("debouncedresize", function() {
				e()
			}), $(".page_aside_toggle").on("click", function(e) {
				e.preventDefault(), $body.toggleClass("page_aside_collapsed")
			})
		}
	},
	_overflow: function() {
		if ($pageOverflow.length) {
			function e() {
				var e = $window.height() - $pageOverflow.offset().top;
				$pageOverflow.children(".uk-overflow-container").length ? $pageOverflow.children(".uk-overflow-container").height(e) : $pageOverflow.height(e)
			}
			e(), $window.on("debouncedresize", function() {
				e()
			})
	...
	search_autocomplete: function() {
		$(".header_main_search_form").children("[data-uk-autocomplete]").on("selectitem.uk.autocomplete", function(e, i, a) {
			console.log(e), console.log(i), console.log(a)
		})
	}
}, altair_md = {
	init: function() {
	...
	card_fullscreen: function() {
		$(".md-card-fullscreen-activate").on("click", function() {
			var a = $(this).closest(".md-card");
			if (!a.hasClass("md-card-fullscreen")) {
				var e = a.height(),
					i = $(this).hasClass("toolbar_fixed"),
					t = a.width(),
					n = $body.scrollTop(),
					s = a.offset();
				a.after('<div class="md-card-placeholder" style="width:' + t + "px;height:" + e + 'px;"/>'), a.addClass("md-card-fullscreen").css({
					width: t,
					height: e,
					left: s.left,
					top: s.top - n
					...
					easing: easing_swiftOut,
					begin: function(e) {
						var i = a.find(".md-card-toolbar");
						i.length ? i.prepend('<span class="md-icon md-card-fullscreen-deactivate material-icons uk-float-left">&#xE5C4;</span>') : a.append('<span class="md-icon md-card-fullscreen-deactivate material-icons uk-position-top-right" style="margin:10px 10px 0 0">&#xE5CD;</span>'), altair_page_content.hide_content_sidebar()
					}
				}).velocity({
					height: "100%",
					width: "100%"
					...
					easing: easing_swiftOut,
					complete: function(e) {
						a.find(".md-card-fullscreen-content").velocity("transition.slideUpBigIn", {
							duration: 400,
							easing: easing_swiftOut,
							complete: function(e) {
								$(window).resize()
			...
			var i = $(".md-card-placeholder"),
				e = i.height(),
				a = i.width(),
				t = $body.scrollTop(),
				n = i.offset().top - t,
				s = i.offset().left,
				r = $(".md-card-fullscreen"),
				o = r.hasClass("mdToolbar_fixed");
			r.velocity({
				height: e,
				width: a
			}, {
				...
				begin: function(e) {
					r.find(".md-card-fullscreen-content").velocity("transition.slideDownOut", {
						duration: 275,
						easing: easing_swiftOut
					}), o && r.removeClass("mdToolbar_fixed")
				},
				complete: function(e) {
				...
				complete: function(e) {
					r.removeClass("md-card-fullscreen").css({
						...
						left: "",
						top: ""
					}), i.remove(), $body.removeClass("md-card-fullscreen-active")
				}
			})
		})
	},
	...
	card_single: function() {
		...
		if (i && $body.hasClass("header_double_height")) {
			function e() {
				var e = $window.height() - (2 * header__main_height + 12);
				i.find(".md-card-content:first").innerHeight(e)
			}
			e(), $window.on("debouncedresize", function() {
				e()
			})
	...
	card_panel: function() {
		$(".md-card-close").on("click", function(e) {
			e.preventDefault();
			var i = $(this).closest(".md-card");
			altair_md.card_show_hide(i, void 0, function() {
				$(i).remove()
			})
			...
			var i = $(this),
				a = i.closest(".md-card");
			$(a).toggleClass("md-card-collapsed").children(".md-card-content").slideToggle("280", bez_easing_swiftOut), i.velocity({
				scale: 0,
				opacity: .2
			}, {
				duration: 280,
				easing: easing_swiftOut,
				complete: function() {
					$(a).hasClass("md-card-collapsed") ? i.html("&#xE313;") : i.html("&#xE316;"), i.velocity("reverse"), $window.resize()
				}
			})
		}), $(".md-card-collapsed").each(function() {
			var e = $(this);
			e.find(".md-card-toggle").html("&#xE313;"), e.children(".md-card-content").hide()
		})
	},
	...
	card_progress: function(e, r, o) {
		(e ? $(e).children(".md-card-toolbar") : $("[data-toolbar-progress]")).each(function() {
			var e = $(this),
				i = r ? parseInt(r) : parseInt(e.attr("data-toolbar-progress")),
				a = e.attr("data-toolbar-progress-steps");
			if (o || a)
				if (66 < i) var t = "#dcedc8";
				else if (33 < i) t = "#ffecb3";
			else t = "#ffcdd2";
			else {
				var n = e.attr("data-toolbar-bg-default");
				if (n) t = n;
				else {
					var t = e.css("backgroundColor");
					e.attr("data-toolbar-bg-default", t)
				}
			}
			r && e.attr("data-toolbar-progress", r);
			var s = "#fff";
			e.css({
				background: "-moz-linear-gradient(left, " + t + " " + i + "%, " + s + " " + i + "%)"
			}).css({
				background: "linear-gradient(to right,  " + t + " " + i + "%, " + s + " " + i + "%)"
			}).css({
				background: "-webkit-linear-gradient(left, " + t + " " + i + "%, " + s + " " + i + "%)"
			})
		})
	},
	list_outside: function() {
		var i = $(".md-list-outside-wrapper"),
			a = i.children(".md-list-outside-inner");
		if (i && $body.hasClass("header_double_height")) {
			function e() {
				var e = $window.height() - (2 * header__main_height + 10);
				i.height(e), a.length && a.height(e)
			}
			e(), $window.on("debouncedresize", function() {
				e()
			}), a.length ? altair_helpers.custom_scrollbar(a) : altair_helpers.custom_scrollbar(i)
		}
	},
	inputs: function(e) {
	...
	checkbox_radio: function(e) {
		(void 0 === e ? $("[data-md-icheck],.data-md-icheck") : $(e)).each(function() {
	...
	fab_speed_dial: function() {
		function t(e) {
			e.closest(".md-fab-wrapper").addClass("md-fab-active"), e.velocity({
				scale: 0
			}, {
				duration: 140,
				easing: easing_swiftOut,
		...
		function n(e) {
			e.closest(".md-fab-wrapper").removeClass("md-fab-active"), e.velocity({
				scale: 0
			}, {
				duration: 140,
				easing: easing_swiftOut,
		...
		$(".md-fab-speed-dial,.md-fab-speed-dial-horizontal").each(function() {
			var e, i = $(this),
				a = i.children(".md-fab").append('<i class="material-icons md-fab-action-close" style="display:none">&#xE5CD;</i>');
			i.is("[data-fab-hover]") ? i.on("mouseenter", function() {
				i.addClass("md-fab-over"), clearTimeout(e), setTimeout(function() {
					t(a)
				}, 100)
			}).on("mouseleave", function() {
				n(a), e = setTimeout(function() {
					i.removeClass("md-fab-over")
				}, 500)
			}) : a.on("click", function() {
				var e = $(this);
				e.closest(".md-fab-wrapper").hasClass("md-fab-active") ? n(e) : t(e)
	...
	fab_toolbar: function() {
		var n = $(".md-fab-toolbar");
		n && (n.children("i").on("click", function(e) {
			e.preventDefault();
			var i = n.children(".md-fab-toolbar-actions").children().length;
			n.addClass("md-fab-animated");
			var a = n.hasClass("md-fab-small") ? 24 : 16,
				t = n.hasClass("md-fab-small") ? 44 : 64;
			setTimeout(function() {
				n.width(i * t + a)
			}, 140), setTimeout(function() {
				n.addClass("md-fab-active")
			}, 420)
		}), $document.on("click scroll", function(e) {
			n.hasClass("md-fab-active") && ($(e.target).closest(n).length || (n.css("width", "").removeClass("md-fab-active"), setTimeout(function() {
				n.removeClass("md-fab-animated")
			}, 140)))
		}))
	},
	fab_sheet: function() {
		var a = $(".md-fab-sheet");
		a && (a.children("i").on("click", function(e) {
			e.preventDefault();
			var i = a.children(".md-fab-sheet-actions").children("a").length;
			a.addClass("md-fab-animated"), setTimeout(function() {
				a.width("240px").height(40 * i + 8)
			}, 140), setTimeout(function() {
				a.addClass("md-fab-active")
			}, 280)
		}), $document.on("click scroll", function(e) {
			a.hasClass("md-fab-active") && ($(e.target).closest(a).length || (a.css({
				height: "",
				width: ""
			}).removeClass("md-fab-active"), setTimeout(function() {
				a.removeClass("md-fab-animated")
			}, 140)))
		}))
	},
	...
	hierarchical_show: function(e) {
		var i = $(".hierarchical_show");
		if (i.length && i.each(function() {
				var e = $(this).attr("data-show-delay") ? parseInt($(this).attr("data-show-delay")) : 0,
					i = $(this),
					a = i.children().length;
				i.children().each(function(e) {
					...
					i.waypoint({
						handler: function() {
							i.addClass("hierarchical_show_inView"), setTimeout(function() {
								i.removeClass("hierarchical_show hierarchical_show_inView fast_animation").children().css({
									"-webkit-animation-delay": "",
									"animation-delay": ""
								})
							}, 100 * a + 1200), this.destroy()
						},
						context: "window",
						offset: "90%"
					})
			...
			var a = $(e).addClass("hierarchical_show hierarchical_show_inView"),
				t = a.children().length;
			...
			}), a.addClass(""), setTimeout(function() {
				a.removeClass("hierarchical_show hierarchical_show_inView fast_animation").children().css({
					"-webkit-animation-delay": "",
					"animation-delay": ""
				})
			}, 100 * t + 1200)
	...
	hierarchical_slide: function(e) {
		var i = $(".hierarchical_slide");
		if (i.length && i.each(function() {
				var e = $(this),
				...
				1 <= a && (i.each(function(e) {
					...
					handler: function() {
						e.addClass("hierarchical_slide_inView"), setTimeout(function() {
							e.removeClass("hierarchical_slide hierarchical_slide_inView"), i.css({
								"-webkit-animation-delay": "",
								"animation-delay": ""
							})
						}, 100 * a + 1200), this.destroy()
			...
			var a = $(e).addClass("hierarchical_slide hierarchical_slide_inView"),
				t = a.attr("data-slide-children") ? a.children(a.attr("data-slide-children")) : a.children(),
				n = t.length;
			a.attr("data-slide-context") && a.closest(a.attr("data-slide-context"))[0];
			console.log(t), 1 <= n && (t.each(function(e) {
				$(this).css({
					"-webkit-animation-delay": 100 * e + "ms",
					"animation-delay": 100 * e + "ms"
				})
			}), setTimeout(function() {
				a.removeClass("hierarchical_slide hierarchical_slide_inView"), t.css({
					"-webkit-animation-delay": "",
					"animation-delay": ""
				})
			}, 100 * n + 1200))
	...
	content_preloader_show: function(e, i, a, t, n) {
		if (!$body.find(".content-preloader").length) {
			var s = isHighDensity() ? "@2x" : "",
				r = (t = t || 48, n = n || 48, "regular" == e ? '<img src="assets/img/spinners/spinner' + s + '.gif" alt="" width="32" height="32">' : '<div class="md-preloader"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="' + n + '" width="' + t + '" viewbox="0 0 75 75"><circle cx="37.5" cy="37.5" r="33.5" stroke-width="8"/></svg></div>'),
				o = i ? "content-preloader-" + i : "";
			(void 0 !== a ? $(a) : $body).append('<div class="content-preloader ' + o + '" style="height:' + n + "px;width:" + t + "px;margin-left:-" + t / 2 + 'px">' + r + "</div>"), setTimeout(function() {
	...
	color_picker: function(e, i, a) {
		if (e) {
			for (var t = randID_generator(), n = i || ["#e53935", "#d81b60", "#8e24aa", "#5e35b1", "#3949ab", "#1e88e5", "#039be5", "#0097a7", "#00897b", "#43a047", "#689f38", "#ef6c00", "#f4511e", "#6d4c41", "#757575", "#546e7a"], s = n.length, r = $('<div class="cp_altair" id="' + t + '"/>'), o = 0; o < s; o++) {
				var d = "md-bg-" == n[o].substring(0, 6) ? "<span data-color=" + n[o] + ' class="' + n[o] + '"></span>' : "<span data-color=" + n[o] + ' style="background:' + n[o] + '"></span>';
				r.append(d)
			}
			return r.append('<input type="hidden">'), $body.on("click", "#" + t + " span", function() {
				$(this).addClass("active_color").siblings().removeClass("active_color").end().closest(".cp_altair").find("input").val($(this).attr("data-color")), "function" == typeof a && a.call(this)
			}), e.append(r)
		}
	},
	retina_images: function() {
		void 0 !== $.fn.dense && $("img").attr("data-dense-cap", 2).dense({
			glue: "@",
			ping: !1
		})
	},
	full_screen: function() {
		var i = $("#full_screen_toggle");
		i.length && i.on("click", function(e) {
			e.preventDefault(), screenfull.enabled && (screenfull.request(), i.children(".material-icons").text("fullscreen_exit"), screenfull.isFullscreen && (screenfull.exit(), i.children(".material-icons").text("fullscreen"))), $window.resize()
		})
	},
	...
	table_tree: function() {
		$table_tree = $(".table_tree"), $table_tree.each(function() {
			var e = $(this),
				i = e.find(".check_childrens"),
				a = e.find(".check_row"),
				t = e.find(".js-toggle-children-row");
			e.find(".show_child_row").nextUntil(".table-parent-row").hide(), i.on("ifChecked", function() {
				...
				e.preventDefault(), $(this).closest("tr").toggleClass("show_child_row").nextUntil(".table-parent-row").toggle()
			})
		})
	},
	ie_fix: function() {
		detectIE() && ($("svg,canvas,video").each(function() {
			...
			$("svg,canvas,video").each(function() {
				var e = $(this),
					i = $(this).attr("height"),
					a = $(this).attr("width");
				i && e.css("height", i), a && e.css("width", a);
	...
	print_page: function() {
		$body.on("click", "#page_print", function(e) {
			e.preventDefault(), UIkit.modal.confirm("Do you want to print this page?", function() {
	...
	scroll_to_top: function() {
		...
		e && $(window).on("scroll", function() {
			120 < $(window).scrollTop() ? e.fadeIn() : e.fadeOut()
		})
	}
}, altair_uikit = {
	...
	accordion_user_selected: function() {
		$("[data-accordion-section-open]").each(function() {
			...
			setTimeout(function() {
				var e = a.data();
				if (sectOpen = e.accordionSectionOpen.toString().split(","), a.children(".uk-accordion-title.uk-active").each(function() {
						$(this).trigger("click")
					}), !e.accordion.options.collapse && 1 < sectOpen.length)
					for (var i = 0; i <= sectOpen.length; i++) a.children(".uk-accordion-title").eq(sectOpen[i] - 1).trigger("click");
				else a.children(".uk-accordion-title").eq(sectOpen[0] - 1).trigger("click")
			}, 220)
		})
	}
};