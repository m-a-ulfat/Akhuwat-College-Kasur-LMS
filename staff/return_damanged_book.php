...
if (isset($_POST["returndamanged"])) {
    ...
    //add a fine
    $fine =
        "INSERT INTO iL_Fines (f_type, f_amt, s_id, s_number, s_name) VALUES (?,?,?,?,?)";
    ...
    if ($stmt && $stmt1 && $fine_stmt && $stmt2) {
        $success =
            "Returned Damanged Book" &&
            header("refresh:1;url=library_operations_damanged_book.php");
    } else {
     ...
     <!--Breadcrums-->
            <div id="top_bar">
                <ul id="breadcrumbs">
                    ...
                    <li><a href="library_operations_damanged_book.php">Library Operations</a></li>
                        ...
                        <form method="post">
                            <div class="uk-grid" data-uk-grid-margin>
                                ...
                                <div class="uk-width-medium-2-2">
                                    ...
                                    $ret =
                                        "SELECT * FROM  iL_ChargesRates WHERE cr_name = 'Damaged Book' ";
                                    $stmt = $mysqli->prepare($ret);
                                    $stmt->execute(); //ok
                                    $res = $stmt->get_result();
                                    while ($row = $res->fetch_object()) { ?>
